...
for _ in tqdm(range(N_FOLDS)):
    ...
    smiles_train, smiles_test, y_train, y_test = train_test_split(smiles_list, y, test_size=0.2, random_state=42, stratify=y)

    model = lq.MorganBinaryClassifier(reduced=REDUCED, time_budget_sec=TIME_BUDGET_SEC, estimator_list=ESTIMATOR_LIST) 
...
with open(os.path.join(root, "..", "results", "internal_classifier_cross_validation.json"), "w") as f:
...
def get_prediction_dataframe(smiles, inchikeys):
    ...
    for yp in y_pred:
        if yp > thr:
            ...
            y_bin += [0]
    df = pd.DataFrame({"inchikey": inchikeys, "smiles": smiles, "y_hat": y_pred, "y_bin": y_bin})
    return df
...
df.to_csv(os.path.join(root, "..", "results", "internal_classifier_all_molecules.csv"), index=False)

print("Predicting on CHEESE molecules")

df = pd.read_csv(os.path.join(root, "..", "results", "cheese_search_smiles_and_inchikey.csv"))
smiles = df["smiles"].tolist()
inchikeys = df["inchikey"].tolist()
df = get_prediction_dataframe(smiles, inchikeys)
y_pred = model.predict_proba(smiles)[:, 1]
df.to_csv(os.path.join(root, "..", "results", "internal_classifier_cheese_search.csv"), index=False)
...
inchikeys = df["inchikey"].tolist()
df = get_prediction_dataframe(smiles, inchikeys)
df.to_csv(os.path.join(root, "..", "results", "internal_classifier_drugbank.csv"), index=False)
