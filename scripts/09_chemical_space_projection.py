...
df_db = pd.read_csv(os.path.join(root, "..", "data", "drugbank_inchikeys_with_cheese_results.csv"))[["inchikey", "smiles"]]
...
dm_db = pd.read_csv(os.path.join(root, "..", "data", "drugbank_cheese_eos39co.csv"))
...
df_db = df_db[df_db["inchikey"].isin(dm_db["key"])].drop_duplicates(subset="inchikey").sort_values(by="inchikey").reset_index(drop=True)
...
dm_db = dm_db[dm_db["key"].isin(df_db["inchikey"])].drop_duplicates(subset="key").sort_values(by="key").reset_index(drop=True)
...
x_db = np.array(dm_db[list(dm_db.columns)[2:]])
x_cs = np.array(dm_cs[list(dm_cs.columns)[2:]])

print("Projecting chemical space")

x = np.concatenate([x_ma, x_cd, x_db, x_cs], axis=0)

inchikeys = list(df_ma["inchikey"]) + list(df_cd["inchikey"])+ list(df_db["inchikey"]) + list(df_cs["inchikey"])
smiles = list(df_ma["smiles"]) + list(df_cd["smiles"]) + list(df_db["smiles"]) + list(df_cs["smiles"])
tot_rank = [-1]*df_ma.shape[0] + [-1]*df_cd.shape[0] + [-1]*df_db.shape[0] + list(df_cs["tot_rank"])
category = ["Manually annotated"]*df_ma.shape[0] + ["ChemDiv Coronavirus"]*df_cd.shape[0] + ["Drugbank"]*df_db.shape[0] + ["CHEESE search"]*df_cs.shape[0]
